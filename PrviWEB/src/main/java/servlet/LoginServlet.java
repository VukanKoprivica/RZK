...
public class LoginServlet extends HttpServlet {
	...
	private static String getLookupName() {
		...
		final String beanName = PlanerUserBean.class.getSimpleName();
		// the remote interface fully qualified class name
		final String interfaceName = PlanerUserBeanRemote.class.getName();
		// let's do the lookup
	...
	private static PlanerUserBeanRemote doLookup() {
		Context context = null;
		PlanerUserBeanRemote bean = null;
		try {
			context = getInitialContext();
			String lookupName = getLookupName();
			System.out.println("JNDI ime:   " + lookupName);
			bean = (PlanerUserBeanRemote) context.lookup(lookupName);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PlanerUserBeanRemote bean = (PlanerUserBeanRemote) request.getSession().getAttribute("bean");
		...
		User user= bean.login(email, pass);
		if(user!= null) {
			List<EventType> tipovi=bean.tipovi();
