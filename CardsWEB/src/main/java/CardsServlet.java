...
import rzk.StatelessCards;
import rzk.StatelessRemote;
...
public class CardsServlet extends HttpServlet {
	...
	private static String getLookupName() {
        ...
        final String moduleName = "CardsEJB";
        ...
        final String beanName = StatelessCards.class.getSimpleName();
        // the remote interface fully qualified class name
        final String interfaceName = StatelessRemote.class.getName();
	...
	private static StatelessRemote doLookup() {
		Context context = null;
		StatelessRemote bean = null;
		try {
			context = getInitialContext();
			String lookupName = getLookupName();
			System.out.println("JNDI ime:   "+lookupName);
			bean = (StatelessRemote) context.lookup(lookupName);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		StatelessRemote bean = (StatelessRemote)request.getSession().getAttribute("bean");
		if(bean == null) {
			bean = doLookup();
		}
		...
		bean.sendQueueMessage(from, to, email, "prazan");
